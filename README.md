...
5. 对于消费指数、医药指数、养老指数等PB从未小于2的品种，我们只在PB,PE均在历史1%的区间内才买入一份，也就是万中挑一；
...
当已买入的品种处于>70%的历史区间时，我们每次都会卖出5份，也就是总持有份额的0.2% (1/250 X 5), 这是根据A股市场的牛熊持续周期比例确定的；
...
像消费、医药、养老等指数，PB历史区间从未<2；所以我们设定了一个`更宽松`的条件，即PB、PE均处于最近5年1%低估范围内，才买入一份；
